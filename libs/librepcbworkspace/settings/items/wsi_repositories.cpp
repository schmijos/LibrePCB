...
#include <librepcbcommon/repository.h>
    ...
    WSI_Base(xmlTagName, xmlElement)
{
    if (xmlElement) {
        ...
        {
            mList.append(new Repository(*child));
        }
    } else {
        // load defaults
        mList.append(new Repository(QUrl("http://api.librepcb.org")));
    }
...
void WSI_Repositories::restoreDefault() noexcept
{
    qDeleteAll(mListTmp);   mListTmp.clear();
    try {
        mListTmp.append(new Repository(QUrl("http://api.librepcb.org")));
    } catch (Exception& e) {
        Q_ASSERT(false);
    }
    updateListWidgetItems();
...
void WSI_Repositories::btnAddClicked() noexcept
{
    try {
        ...
        mListTmp.append(new Repository(QUrl::fromUserInput(input)));
        updateListWidgetItems();
        mLineEdit->setText("");
    } catch (Exception& e) {
        QMessageBox::critical(mWidget.data(), tr("Invalid URL"), e.getUserMsg());
    }
}
...
XmlDomElement* WSI_Repositories::serializeToXmlDomElement() const throw (Exception)
{
    ...
    foreach (const Repository* repository, mList) {
        root->appendChild(repository->serializeToXmlDomElement());
    }
    return root.take();
}
