...
int WorkspaceLibraryDb::rescan() throw (Exception)
{
    ...
    SQLiteDatabase::TransactionScopeGuard transactionGuard(*mDb); // can throw

    // clear all tables
    clearAllTables();

    // scan local libraries
    foreach (const QSharedPointer<Library>& lib, mWorkspace.getLocalLibraries()) {
        count += addCategoriesToDb<ComponentCategory>(  lib->searchForElements<ComponentCategory>(),    "component_categories", "cat_id");
        count += addCategoriesToDb<PackageCategory>(    lib->searchForElements<PackageCategory>(),      "package_categories",   "cat_id");
        count += addElementsToDb<Symbol>(               lib->searchForElements<Symbol>(),               "symbols",              "symbol_id");
        count += addElementsToDb<Package>(              lib->searchForElements<Package>(),              "packages",             "package_id");
        count += addElementsToDb<Component>(            lib->searchForElements<Component>(),            "components",           "component_id");
        count += addDevicesToDb(                        lib->searchForElements<Device>(),               "devices",              "device_id");
    }

    // scan remote libraries
    foreach (const QSharedPointer<Library>& lib, mWorkspace.getRemoteLibraries()) {
        count += addCategoriesToDb<ComponentCategory>(  lib->searchForElements<ComponentCategory>(),    "component_categories", "cat_id");
        count += addCategoriesToDb<PackageCategory>(    lib->searchForElements<PackageCategory>(),      "package_categories",   "cat_id");
        count += addElementsToDb<Symbol>(               lib->searchForElements<Symbol>(),               "symbols",              "symbol_id");
        count += addElementsToDb<Package>(              lib->searchForElements<Package>(),              "packages",             "package_id");
...
int WorkspaceLibraryDb::addCategoriesToDb(const QList<FilePath>& dirs,
    const QString& tablename, const QString& id_rowname) throw (Exception)
{
    int count = 0;
...
template <typename ElementType>
int WorkspaceLibraryDb::addElementsToDb(const QList<FilePath>& dirs,
    const QString& tablename, const QString& id_rowname) throw (Exception)
{
    int count = 0;
    foreach (const FilePath& filepath, dirs) {
...
int WorkspaceLibraryDb::addDevicesToDb(const QList<FilePath>& dirs,
    const QString& tablename, const QString& id_rowname) throw (Exception)
{
    int count = 0;
    foreach (const FilePath& filepath, dirs)
...
{
    // internal
    mDb->clearTable("internal");

    // component categories
    mDb->clearTable("component_categories_tr");
    mDb->clearTable("component_categories");

    // package categories
    mDb->clearTable("package_categories_tr");
    mDb->clearTable("package_categories");

    // symbols
    mDb->clearTable("symbols_tr");
    mDb->clearTable("symbols_cat");
    mDb->clearTable("symbols");

    // packages
    mDb->clearTable("packages_tr");
    mDb->clearTable("packages_cat");
    mDb->clearTable("packages");

    // components
    mDb->clearTable("components_tr");
    mDb->clearTable("components_cat");
    mDb->clearTable("components");

    // devices
    mDb->clearTable("devices_tr");
    mDb->clearTable("devices_cat");
    mDb->clearTable("devices");
}
